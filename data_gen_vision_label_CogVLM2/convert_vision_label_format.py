...
def process_data(file_path, ids, split):
    ...
    for data in labels:
        if data["image_id"] in ids:
            yes_probs = data["logits"]["Yes"] / (
                data["logits"]["Yes"] + data["logits"]["No"]
            )
            no_probs = data["logits"]["No"] / (
                data["logits"]["Yes"] + data["logits"]["No"]
            )
            if split == "train":
...
if __name__ == "__main__":
    ...
    save_results_to_json(
        results,
        f"../{dataset_name}_data/data_gen_output/{dataset_name}_image_only_pred_cogvlm2.json",
    )
