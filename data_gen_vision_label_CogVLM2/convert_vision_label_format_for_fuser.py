...
def apply_thresholds(results: Dict, thresholds: List[float]) -> Dict[float, float]:
    return {
        threshold: f1_score(
            ...
            [1 if value['logits']['Yes'] >= threshold else 0 for value in results.values() if value['gth'] is not None]
        )
...
def select_top_percent_as_one(results, percentage):
    sorted_logits = sorted(results.items(), key=lambda x: x[1]['logits']['Yes'], reverse=True)
