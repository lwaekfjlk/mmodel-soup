...
def evaluate(model, data_loader, tokenizer, device, config):
    ...
    for images, text, targets, image_ids in metric_logger.log_every(
        ...
        for image_id, text, pred, target, logits in zip(
            image_ids, text, pred_class.tolist(), targets.tolist(), prediction.tolist()
        ):
            eval_results.append(
                {
                    "image_id": image_id,
    ...
    precision, recall, f1 = calc_precision_recall_f1(
        np.array([res["pred"] for res in eval_results]),
        np.array([res["target"] for res in eval_results]),
    )
    print(
        "Averaged stats:",
...
def main(args, config):
    ...
    datasets = create_dataset("mustard", config)
    ...
    for epoch in range(0, max_epoch):
        ...
        test_stats, test_results = evaluate(
            model, test_loader, tokenizer, device, config
        )

        with open(
            os.path.join(args.output_dir, "mustard_vision_text_logits.jsonl"), "w"
        ) as f:
            f.write("")

        with open(
            os.path.join(args.output_dir, "mustard_vision_text_logits.jsonl"), "a"
        ) as f:
