...
def get_mmsd_dataloader(args, tokenizer, image_processor, split):
    if split == "train":
        dataset = MMSDDataset(args.train_path, args.image_data_path, tokenizer, image_processor, args.max_length)
        return DataLoader(dataset, batch_size=args.batch_size, shuffle=True, collate_fn=mmsd_collate)
    elif split == "val":
        dataset = MMSDDataset(args.val_path, args.image_data_path, tokenizer, image_processor, args.max_length)
        return DataLoader(dataset, batch_size=args.val_batch_size, shuffle=False, collate_fn=mmsd_collate)
    elif split == "test":
        dataset = MMSDDataset(args.test_path, args.image_data_path, tokenizer, image_processor, args.max_length)
