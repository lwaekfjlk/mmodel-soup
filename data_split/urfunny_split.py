...
def read_preds():
    text_only_pred = read_json_file(
        "../urfunny_data/data_gen_output/urfunny_text_only_pred_qwen2.json"
    )
    vision_only_pred = read_json_file(
        "../urfunny_data/data_gen_output/urfunny_image_only_pred_cogvlm2.json"
    )
...
def main():
    for split in ["train", "test", "val"]:
        ...
        save_dataset(
            f"../urfunny_data/data_split_output/urfunny_R_dataset_{split}_cogvlm2_qwen2.json",
            R_dataset,
        )

        AS_dataset = construct_subset(AS_ids, train_dataset)
        save_dataset(
            f"../urfunny_data/data_split_output/urfunny_AS_dataset_{split}_cogvlm2_qwen2.json",
            AS_dataset,
        )

        U_dataset = construct_subset(U_ids, train_dataset)
        save_dataset(
            f"../urfunny_data/data_split_output/urfunny_U_dataset_{split}_cogvlm2_qwen2.json",
            U_dataset,
        )
