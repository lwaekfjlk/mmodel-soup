...
def read_preds(split):
    text_only_pred = read_json_file(
        "../urfunny_data/data_gen_output/urfunny_text_only_pred_qwen2_for_fuser.json"
    )
    vision_only_pred = read_json_file(
        "../urfunny_data/data_gen_output/urfunny_image_only_pred_cogvlm2_for_fuser.json"
    )
...
def main():
    for split in ["train", "val", "test"]:
        ...
        train_dataset = read_json_file(
            f"../urfunny_data/data_raw/urfunny_dataset_{split}.json"
        )
        ...
        save_dataset(
            f"../urfunny_data/data_split_output/urfunny_R_dataset_{split}_cogvlm2_qwen2_for_fuser.json",
            R_dataset,
        )

        AS_dataset = construct_subset(AS_ids, train_dataset)
        save_dataset(
            f"../urfunny_data/data_split_output/urfunny_AS_dataset_{split}_cogvlm2_qwen2_for_fuser.json",
            AS_dataset,
        )

        U_dataset = construct_subset(U_ids, train_dataset)
        save_dataset(
            f"../urfunny_data/data_split_output/urfunny_U_dataset_{split}_cogvlm2_qwen2_for_fuser.json",
            U_dataset,
        )
