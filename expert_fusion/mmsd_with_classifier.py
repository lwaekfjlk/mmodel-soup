...
class MMSDDataset(Dataset):
    def __init__(self, split, image_data_path, tokenizer, image_processor, max_length=512):
        ...
        with open("../mmsd_data/data_raw/mmsd_dataset_test.json") as f:
            self.gth = {id: 1 if data["label"] else 0 for id, data in json.load(f).items()}
        
        with open("../mmsd_data/data_raw/mmsd_dataset_train.json") as f:
            self.gth.update({id: 1 if data["label"] else 0 for id, data in json.load(f).items()})
        
        with open("../mmsd_data/data_raw/mmsd_dataset_val.json") as f:
            self.gth.update({id: 1 if data["label"] else 0 for id, data in json.load(f).items()})
