...
class MustardDataset(Dataset):
    ...
    def load_dataset(self, dataset_path_dict):
        raw_dataset = {}
        for type, dataset_path in dataset_path_dict.items():
            with open(dataset_path) as f:
                raw_dataset[type] = json.load(f)

        overall_dataset = []        
        for type, dataset in raw_dataset.items():
