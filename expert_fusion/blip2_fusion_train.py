...
def evaluate(tokenizer, model, dataloader, device):
    ...
    rus_logits = {}
    token_ids = {
        token: tokenizer.convert_tokens_to_ids(tokenizer.tokenize(token))[0]
        for token in ["R", "U", "S"]
    }
    ...
    for batch in tqdm(dataloader, desc="Evaluating", leave=False):
        input_ids, attention_mask, images, labels = (
            batch[key].to(device)
            for key in ["input_ids", "attention_mask", "image", "label"]
        )
        ids = batch["id"]
        ...
        with torch.no_grad():
            ...
            rus_logits_batch = torch.stack(
                [logits[:, token_ids[token]] for token in ["R", "U", "S"]], dim=-1
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_data_path", type=str, required=True, help="Path to the image data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--load_model_name", type=str, help="Path to load the model from"
    )
    parser.add_argument(
