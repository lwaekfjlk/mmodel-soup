...
import argparse
from irfl_dataset_idioms import CustomDataset, custom_collate
...
def train(args, model, train_dataloader, val_dataloader, tokenizer, device, epochs=50):
    ...
    a_token_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize("0"))[0]
    b_token_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize("1"))[0]
    c_token_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize("2"))[0]
    d_token_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize("3"))[0]
    e_token_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize("4"))[0]
...
if __name__ == '__main__':
    ...
    # Define arguments
    parser.add_argument('--train_path', type=str, default="/root/haofeiy/mmodel-soup/expert_mistral/mistral_idioms.json", help='Path to training dataset')
    parser.add_argument('--val_path', type=str, default="/root/haofeiy/mmodel-soup/expert_mistral/mistral_idioms.json",help='Path to validation dataset')
    parser.add_argument('--train_batch_size', type=int, default=1, help='Batch size for training')
    parser.add_argument('--val_batch_size', type=int, default=2, help='Batch size for validation')
    parser.add_argument('--max_length', type=int, default=1024, help='Maximum sequence length')
    parser.add_argument('--learning_rate', type=float, default=5e-5, help='Lefullarning rate for the optimizer')
    ...
    parser.add_argument('--save_path', type=str, default='./idiom_mistral_model', help='Path to save the trained model')
